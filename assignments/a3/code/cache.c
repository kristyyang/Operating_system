...
static long cache_line_retrieve_data(cache_line_t *cache_line, size_t offset) {
    ...
    int64_t i = cache_line->data[offset];
...
static cache_line_t *cache_set_find_matching_line(cache_t *cache, cache_set_t *cache_set,
     ...
     */
// Added to remove warning; remove once function is implemented.
}
...
long cache_read(cache_t *cache, long *address) {
    ...
    uintptr_t index = ((uintptr_t)address >> cache->cache_index_shift) & cache->cache_index_mask;
    uintptr_t offset = (uintptr_t)address & cache->block_offset_mask;
    ...
    if(!cache_line_check_validity_and_tag(cache_line, tag)){
        cache->miss_count++;
        cache_line = cache_set_add(cache, cache_set, (uintptr_t) address, tag);

    }
    return cache_line_retrieve_data(cache_line, offset);
}
